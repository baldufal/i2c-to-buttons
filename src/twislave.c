...
* - Pure read: 4 bytes can be read, starting from address 0x0.
*    The data consists of one status byte (at 0x0) followed by two data bytes and a checksum.
        ...
        case TW_SR_DATA_ACK:
            ...
            if (buffer_addr == 0xFF) {
                ...
                if (buffer_addr == 0x0) {
                    ...
                    // Bit 1 (rightmost) high = readout pending -> allow setting only (cleared by mc)
        ...
        case TW_ST_DATA_ACK:
            ...
            if (buffer_addr < i2c_buffer_size) {
                ...
                if (buffer_addr == (i2c_buffer_size - 1)) {
                    // Reading last byte triggers readout.
